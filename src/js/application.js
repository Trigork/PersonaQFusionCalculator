...
$(document).ready(function() {
    ...
    $("#arcana-navbar").empty();
    $.each(personaQ.arcana, function(i) {
    ...
    if (r) {
        ...
        $.each(personaQ.personas.sort(personaSortLevelAsc), function(i) {
            if (this.arcana === r) {
                ...
                if (this.level > avg && (typeof activePersona === "undefined") && [a, b, c].indexOf(this) < 0 && this.type != "special") {
                    activePersona = this.name;
                    personas[personas.length - 1].active = true;
...
function sortPersonas(a, b, c) {
    ...
    if (p[1].level === p[2].level) {
        //console.log(new Array(b, c).sort(personaSortArcanaDesc));
        p = new Array(a).concat(new Array(b, c).sort(personaSortArcanaDesc));
